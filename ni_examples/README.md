...
```bash
cd src
./vellvm -interpret-obs-secret <secret_value> ../ni_examples/<file>.ll
```

The output shows observations between `---OBS_TRACE_BEGIN---` and `---OBS_TRACE_END---`:
...
```bash
./vellvm -interpret-obs-secret 90 ../ni_examples/leak_test2_nested_branch.ll   # Br(T), Br(T) → ret 3
./vellvm -interpret-obs-secret 60 ../ni_examples/leak_test2_nested_branch.ll   # Br(T), Br(F) → ret 2
./vellvm -interpret-obs-secret 30 ../ni_examples/leak_test2_nested_branch.ll   # Br(F), Br(T) → ret 1
./vellvm -interpret-obs-secret 10 ../ni_examples/leak_test2_nested_branch.ll   # Br(F), Br(F) → ret 0
```
...
```bash
./vellvm -interpret-obs-secret 0 ../ni_examples/leak_test3_memory_only.ll
# Trace: ...S(298), S(302), S(306), S(310), L(298)   ← loads arr[0]

./vellvm -interpret-obs-secret 2 ../ni_examples/leak_test3_memory_only.ll
...
```bash
./vellvm -interpret-obs-secret 100 ../ni_examples/leak_test4_branch_and_memory.ll
# Trace: ...S(298), S(302), Br(true), L(298)    ← true branch, loads arr[0]

./vellvm -interpret-obs-secret 0 ../ni_examples/leak_test4_branch_and_memory.ll
...
```bash
./vellvm -interpret-obs-secret 42 ../ni_examples/leak_test5_no_leak.ll
# Trace: ...S(298), S(298), L(298)

./vellvm -interpret-obs-secret 99 ../ni_examples/leak_test5_no_leak.ll
# Trace: ...S(298), S(298), L(298)
```
...
```bash
./vellvm -interpret-obs-secret 5 ../ni_examples/leak_test6_branch_diff_alloca.ll
