...
(* Step an [interp_mcfg4_exec_obs]-shaped itree.
   Old base L4 has 5 events (no LLVMExcE). The RetF result type wraps an
   extra [list Z] (the obs trace) between [store_id] and the stack pair. *)
let rec step_obs m =
  let open ITreeDefinition in
  match observe m with
  | TauF x -> step_obs x
...
(* Step the taint-augmented itree. Same shape as [step_obs] but RetF
   carries an extra [tstate], so the result is [(obs, tstate, dvalue)]. *)
