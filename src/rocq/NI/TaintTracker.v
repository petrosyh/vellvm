...
Section ExpTaint.
  ...
  Fixpoint calc_taint_exp (e : @exp T) (tr : treg_map) : taint :=
    ...
    | EXP_Integer _ | EXP_Float _ | EXP_Bool _
    | EXP_Null     | EXP_Zero_initializer
    ...
    | EXP_Vector _ fields =>
        List.fold_left
          (fun acc '(_, e) => join_taints acc (calc_taint_exp e tr))
          fields []
    | EXP_Splat (_, e) => calc_taint_exp e tr
    | OP_IBinop _ _ v1 v2
    ...
    | OP_FCmp _ _ v1 v2 =>
        join_taints (calc_taint_exp v1 tr) (calc_taint_exp v2 tr)
    | OP_Fneg _ (_, v) => calc_taint_exp v tr
...
Module Make (LP : LLVMParams) (MEM : Memory LP).
  ...
  (** Denote one instruction AND thread the taint state.
      Mirrors [denote_instr]'s triple-with-metadata input shape.
      Load and Store are duplicated so we can grab the concrete address
      [da] for memory-taint lookup / update; every other case
      delegates to [denote_instr] and applies the pure update. *)
  Definition denote_instr_taint
    (i : instr_id * instr dtyp * list (metadata dtyp))
    (varargs : option ADDR.addr)
    (ts : tstate) : itree (instr_E dvalue uvalue) tstate :=
    let '(iid, instr_body, md) := i in
    let tr := ts_tregs ts in
    let pc := ts_tpc ts in
    let ob := ts_tobs ts in
    let tm := ts_tmem ts in
    ...
    | IVoid _, INSTR_Store (dt, val) (du, ptr) _ =>
        ...
        (* The store address is observable (the Store reveals that this
           specific cell was written). *)
        let obs_taint :=
          join_taints (join_taints (join_taints addr_self ptr_taint) pc) ob in
    ...
    | _, _ =>
        denote_instr (iid, instr_body, md) varargs ;;
  ...
  Definition denote_block_taint (b : block dtyp) (bid_from : block_id)
    ...
    : itree (instr_E dvalue uvalue) (tstate * (block_id + uvalue)) :=
    denote_phis bid_from (blk_phis b) ;;
    let ts1 := List.fold_left
                 (fun ts' '(id, p, _md) => taint_phi_gen id p bid_from ts')
                 (blk_phis b) ts in
    ts2 <- denote_code_taint (blk_code b) varargs ts1 ;;
    let '(_iid, term, _md) := blk_term b in
    let ts3 := taint_term_gen term ts2 in
    r <- denote_terminator (blk_term b) ;;
    ret (ts3, r).

  Definition denote_ocfg_taint (bks : ocfg dtyp) (varargs : option ADDR.addr)
    : (tstate * (block_id * block_id))
      -> itree (instr_E dvalue uvalue)
  ...
  Definition denote_cfg_taint (f : cfg dtyp) (varargs : option ADDR.addr)
    (ts : tstate)
    : itree (instr_E dvalue uvalue) (tstate * uvalue) :=
  ...
  (** Denote a function with taint tracking.
      No [sources] parameter: every variable / cell is automatically its
      own source via the lookup defaults. Function parameters get
      their own identities the first time they're read. *)
  Definition denote_function_taint
    (df : definition dtyp (cfg dtyp)) (args : list uvalue)
    : itree (L0' dvalue uvalue) (tstate * uvalue) :=
    varg <- push_call_frame df args ;;
    '(ts_final, rv) <- translate (@instr_to_L0' dvalue uvalue)
